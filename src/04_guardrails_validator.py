...
class PIIDetector(Validator):
    """Phát hiện và redact PII: EMAIL, PHONE, SSN, CREDIT_CARD."""
    ...
    def validate(self, value: str, metadata: dict):
        ...
        if found_pii:
            ...
            # FailResult + fix_value: on_fail=FIX sẽ thay output bằng fix_value
            return FailResult(
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            return FailResult(
                error_message="JSON lỗi nhưng đã sửa được",
                fix_value=json.dumps(parsed, indent=2),
            )
        except json.JSONDecodeError as e:
            return FailResult(error_message=f"JSON không hợp lệ sau khi sửa: {e}", fix_value=json.dumps({"error": "invalid_json", "message": "Could not repair JSON"}, indent=2))
